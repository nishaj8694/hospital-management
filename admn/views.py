...
def sales_search(request):
    if request.method == 'POST':
        ...
        orders_by_month = orders.values('order_date__year', 'order_date__month').annotate(total_revenue=Sum('Total_Price')).order_by('order_date__year', 'order_date__month')
        ...
        if month:
            ...
            yearly_revenues = {order['order_date__year']: 0 for order in orders_by_month}
            for order in orders_by_month:
                yearly_revenues[order['order_date__year']] += order['total_revenue']

            for year, total_revenue in yearly_revenues.items():
                sales_data.append({
                    'year': year,
...
def today_report(request):
    ...
    orders_today = Order.objects.filter(order_date__date=today) \
        .values('order_date__date').annotate(total_revenue=Sum('Total_Price'))
    sales_data = []
...
def admin_ord_c(request):    
    order_list=Order.objects.prefetch_related(Prefetch('order_items',queryset=orderItem.objects.all())).filter(status='deliverd' or 'cancel')
